  ...
  def _data_contract_specification(uri, page, size, filter):
    ...
    with db.session() as session:
      query = """
        ...
        with dc
        match (dc)-[r1]->(bcp:BiomedicalConceptProperty)<-[r2]-(bc:BiomedicalConcept)
        match (dc)-[r3:INSTANCES_REL]->(sai:ScheduledActivityInstance)
        match (sai)-[r4:RELATIVE_FROM_SCHEDULED_INSTANCE_REL]-(t_from:Timing)
        match (t_from)-[r5:RELATIVE_TO_FROM_REL]->()
        match (t_from)<-[r6:TIMINGS_REL]-(tl:ScheduleTimeline)
        match (t_from)-[r7:RELATIVE_TO_FROM_REL]->(rel:Code)
        match (t_from)-[r8:TYPE_REL]->(type:Code)
        // optional match (sai)-[:RELATIVE_TO_SCHEDULED_INSTANCE_REL]-(t_to:Timing)
        optional match (sai)-[r9:ENCOUNTER_REL]-(e1:Encounter)
        optional match (t_from)<-[r10:SCHEDULED_AT_REL]-(e2:Encounter)
        optional match (bcp)-[r11:RESPONSE_CODES_REL]->(r99:ResponseCode)-[r12:CODE_REL]->(term:Code)
